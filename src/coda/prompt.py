...
def _ctx_namespace(ctx: Context) -> str:
    return (
        "## How you read and write data — the turn boundary\n\n"
        "**You (the LLM) and the sandbox are not in the same place in "
        "time.** You write a whole assistant message — possibly with "
        "several `​```python` blocks — in one shot. Then the sandbox "
        "runs those blocks in order. Then you get a new user message "
        "containing the results. You cannot read a result *inside* the "
        "same turn that produced it: by the time the block runs, your "
        "message is already finished and the LLM call is over.\n\n"
        "Concretely, in the turn you are currently writing:\n\n"
        "- A call like `user = airline.get_user_details(...)` returns a "
        "real Python value to the sandbox, but you (the LLM) do not see "
        "that value until the *next* user message.\n"
        "- `reply = respond(\"What's your user id?\")` returns the "
        "customer's actual reply to the sandbox — but again, you don't "
        "see what they said until the next turn. The customer's reply "
        "also appears in the conversation history automatically; you do "
        "not need to mirror it anywhere.\n"
        "- Any later code block in the same message that names something "
        "produced by an earlier block — an id, a list, a field value — "
        "is being written from your imagination of what *might* be there. "
        "If you write `airline.update_reservation_flights("
        "reservation_id='???', ...)` in a block after the fetch, the "
        "id is something you made up. The tool call will fail with "
        "\"not found\" or, worse, succeed against the wrong record.\n\n"
        "**The rule.** If a later block in this message needs to *look "
        "at* the result of an earlier block — filter it, branch on it, "
        "name an id from it — STOP after the earlier block. End the "
        "turn. The result lands in `<ctx_delta>` in the next user "
        "message; only then can you write code that depends on it.\n\n"
        "**Batching is safe and encouraged when blocks are independent.** "
        "If you already know all the arguments — for example, "
        "`ctx.business_ids` was populated in a *prior* turn and you now "
        "want to call `update_reservation_flights` on each — emit all "
        "the writes in one turn. None of them needs another's result. "
        "Same for `respond(...)` + an unrelated fetch in the same turn.\n\n"
        "## Inspecting values: ctx vs. globals vs. print\n\n"
        "**`ctx.NAME = value` is your print().** Assigning to ctx tells "
        "the harness to pretty-print the value back to you in the next "
        "user message inside `<ctx_delta>`. You see it laid out across "
        "multiple lines, the same way `print(value)` would render it — "
        "but the value also persists for later turns, costs no extra "
        "tokens on subsequent turns (the delta only renders entries that "
        "*changed* this turn), and is tracked with a source expression "
        "for requery.\n\n"
        ...
        "directly. Use plain globals for large records you want to "
        "filter or query in *later* turns but don't need to *look at*.\n\n"
        "2. **`ctx.*` attributes** (`ctx.user_summary = {...}`) — also "
        "persist across turns, AND the value is rendered to you in the "
        "*next* turn via `<ctx_delta>`. Use ctx for any value you want "
        "to verify, summarize, or carry forward.\n\n"
        "Workflow:\n\n"
        f"{CODE_FENCE}python\n"
        "# Turn N — fetch, then stop. Don't write code below that uses `user`.\n"
        "user = airline.get_user_details(user_id='abc')\n"
        "ctx.user = user\n"
        f"{CODE_FENCE}\n\n"
        "After this runs, the next user message contains:\n\n"
        f"{CODE_FENCE}\n"
        "<ctx_delta>\n"
        "# added: ctx.user\n"
        "ctx.user = {\n"
        ...
        f"{CODE_FENCE}\n\n"
        f"{CODE_FENCE}python\n"
        "# Turn N+1 — NOW you can see ctx.user and act on real ids.\n"
        "ctx.business_ids = [\n"
        "    rid for rid in ctx.user['reservations']\n"
        "    # filter as needed\n"
        "]\n"
        f"{CODE_FENCE}\n\n"
        "Use the ctx_delta view to verify your fetches and computations — "
        "the way you would have used print output. If a value isn't what "
        "you expect, fix the code in your next turn.\n\n"
